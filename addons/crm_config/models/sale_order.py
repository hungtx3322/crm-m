...
class SaleOrder(models.Model):
    ...
    x_qty_with_uom = fields.Char(string='Tổng số lượng', compute='_compute_total_qty_with_uom', store=True)

    @api.depends('order_line.product_uom_qty', 'order_line.product_uom_id')
    def _compute_total_qty_with_uom(self):
        for order in self:
            if order.order_line:
                total_qty = sum(order.order_line.mapped('product_uom_qty'))
                uom_name = order.order_line[0].product_uom_id.name
                order.x_qty_with_uom = f"{total_qty} {uom_name}"
            else:
                order.x_qty_with_uom = "0"
        ...
        def _convert_group(n):
            ...
            if h > 0:
                res += units[h] + " trăm "
                    ...
                    t > 0 or u > 0):  # Avoid "không trăm" at the very beginning of the whole number if not needed
